  ...
  return (
    <main className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 className="tv-adm-page-title">Contacts</h1>
          <p className="text-[var(--tv-text-muted)] text-sm mt-1">View and respond to customer inquiries and support messages</p>
        </div>
        <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2.5">
          <button className="w-full sm:w-auto flex items-center justify-center gap-2 bg-[var(--tv-slate)] hover:bg-[rgba(241,243,241,0.09)] text-[var(--tv-text)] px-4 py-2.5 rounded-xl transition-all font-medium border border-[var(--tv-rule)]">
        ...
        {selectedContact ? (
          <div className="p-4 sm:p-6 space-y-6">
            <div className="flex items-center justify-between border-b border-[var(--tv-rule)] pb-4">
              <div>
                <h2 className="tv-adm-panel-title">{selectedContact.name || 'Contact Details'}</h2>
                <p className="text-sm text-[var(--tv-text-muted)] mt-1">Contact ID: {selectedContact.id}</p>
              </div>
              <button
                type="button"
            ...
            <section className="space-y-3">
              <h3 className="tv-adm-label">Contact Information</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-[var(--tv-slate)] rounded-lg p-4">
                  ...
                  <p className="mt-1 text-[var(--tv-text)]">{selectedContact.name}</p>
                </div>
                <div className="bg-[var(--tv-slate)] rounded-lg p-4">
                  <p className="text-xs uppercase tracking-wide text-[var(--tv-text-muted)]">Email</p>
                  <p className="mt-1 text-[var(--tv-text)] break-all">{selectedContact.email}</p>
                </div>
                <div className="bg-[var(--tv-slate)] rounded-lg p-4">
                  <p className="text-xs uppercase tracking-wide text-[var(--tv-text-muted)]">Phone</p>
                  <p className="mt-1 text-[var(--tv-text)]">{selectedContact.phone}</p>
                </div>
                <div className="bg-[var(--tv-slate)] rounded-lg p-4">
                  <p className="text-xs uppercase tracking-wide text-[var(--tv-text-muted)]">Subject</p>
                  <p className="mt-1 text-[var(--tv-text)]">{selectedContact.subject || '-'}</p>
                </div>
              </div>
            </section>

            <section className="space-y-3">
              <h3 className="tv-adm-label">Source</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-[var(--tv-slate)] rounded-lg p-4">
                  <p className="text-xs uppercase tracking-wide text-[var(--tv-text-muted)]">Source</p>
                  <p className="mt-1 text-[var(--tv-text)]">{selectedContact.source || 'Website'}</p>
                </div>
                <div className="bg-[var(--tv-slate)] rounded-lg p-4">
                  <p className="text-xs uppercase tracking-wide text-[var(--tv-text-muted)]">Date Received</p>
                  <p className="mt-1 text-[var(--tv-text)]">{selectedContact.createdAt || '-'}</p>
                </div>
              </div>
            </section>

            <section className="space-y-3">
              <h3 className="tv-adm-label">Message</h3>
              <div className="bg-[var(--tv-slate)] rounded-lg p-4">
                <p className="text-[var(--tv-text)] whitespace-pre-line">{selectedContact.comments || '-'}</p>
              </div>
            </section>
          </div>
        ) : null}
