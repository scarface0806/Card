...
/* The staggered fade-up variants that used to live here were removed with the
   motion wrappers - see the note on the LEFT column below. The hero is the LCP
   region; it must be visible at first paint, not after a bundle loads. */
    ...
    <section className="tv-hero relative w-full min-h-screen flex items-center overflow-visible">
      <div className="site-container py-24 lg:py-28 overflow-visible">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          {/* LEFT - Content.
              PLAIN ELEMENTS, NOT motion.*. This block used to fade up from
              opacity 0 on mount. Chrome does not count an element at opacity 0
              towards LCP, so the largest text on the page only became eligible
              once framer-motion had downloaded, hydrated and run - measured at
              5.6s LCP against a 1.2s FCP, a 4.4s gap in which the hero was
              painted but invisible. Rendering at the final state makes LCP
              land with the first paint. */}
          <div className="lg:col-span-5 lg:pr-4">
            <p className="tv-eyebrow mb-7">
              NFC Business Cards
            </p>

            {/* Measure is capped at 15ch so this always breaks into two clean
                lines instead of four with a single word stranded. */}
            <h1 className="tv-display tv-measure-display mb-6">
              One tap. Your entire profile.
            </h1>

            <p className="tv-lead tv-measure-lead mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-10">
              {/* Plain Links. The hover lift these had is already in the
                  .tv-btn CSS (transform: translateY(-2px) on :hover), so the
                  motion wrappers were paying a JS cost to duplicate a rule the
                  stylesheet applies for free - on the two largest elements in
                  the LCP window. */}
              <Link
          ...
          {/* RIGHT - The object itself. Also plain: this held the card image,
              which is the LCP candidate on wider viewports, behind the same
              opacity-0 mount animation. */}
